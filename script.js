...
(function () {
  ...
  // Deep links only choose the initial panel; scrolling remains owned by this controller.
  var entryId = sessionStorage.getItem('e4laServiceEntry');
  if (mobile.matches && entryId) {
    sessionStorage.removeItem('e4laServiceEntry');
    var entryIndex = panels.findIndex(function (panel) { return panel.id === entryId; });
    if (entryIndex >= 0) requestAnimationFrame(function () { enterServices(entryIndex); });
  }
})();
/* Robot handoff is now owned by the services controller above. */
/* FAQ accordion (post-booking section, index.html). One item at a time. */
(function(){
  var questions = Array.prototype.slice.call(document.querySelectorAll('.faq-item__q'));
...
(() => {
  ...
  const choices = modal.querySelectorAll('.booking-choice input[type="checkbox"]');
  ...
  function isStep1Valid(show) {
    ...
    if (group && error) { group.classList.toggle('is-invalid', show && !ok); error.textContent = show && !ok ? 'Please choose at least one focus area.' : ''; error.hidden = !show || ok; }
    return ok;
  }
  ...
  function renderExtraSummaryRows() {
    ...
    if (!list) return;
    const rows = [['goal', 'Selected Focuses', bookingState.bookingReason], ['name', 'Name', bookingState.personalInfo.fullName], ['context', 'Optional Context', optionalSummary()]];
  ...
  modal.querySelectorAll('input[name="booking-focus"]').forEach((choice) => { const card = choice.closest('.booking-choice'); card?.setAttribute('tabindex', '0'); card?.addEventListener('keydown', (event) => { if ((event.key === 'Enter' || event.key === ' ') && !event.defaultPrevented) { event.preventDefault(); choice.checked = !choice.checked; choice.dispatchEvent(new Event('change', { bubbles: true })); } }); choice.addEventListener('change', () => { bookingState.bookingReason = Array.from(modal.querySelectorAll('input[name="booking-focus"]:checked')).map((input) => input.value).join(' | '); card?.classList.toggle('is-selected', choice.checked); saveBookingState(); updateSummary(); updateButtons(); }); });
...
(() => {
  ...
  let navigationLocked = false;
  let confirmLocked = false;
  let createdBooking = bookingState.createdBooking || null;
  let activeFinalFiles = [];
  let lastAppointmentEditTrigger = null;
  let appointmentSaveLocked = false;
  const openers = document.querySelectorAll('[data-booking-open]');
  ...
  const successSessionType = modal.querySelector('[data-success-session-type]');
  const successPrepCopy = modal.querySelector('[data-success-prep-copy]');
  const successDetails = modal.querySelector('[data-success-details]');
  const successMessage = modal.querySelector('[data-success-message]');
  const returnHomeButton = modal.querySelector('[data-return-home]');
  const addCalendarButton = modal.querySelector('[data-add-calendar]');
  const policyInput = modal.querySelector('#booking-policy');
  ...
  function isStep1Valid(show) { const ok = Boolean(bookingState.bookingReason); const group = modal.querySelector('[data-booking-panel="1"] .booking-choice-grid'); let error = group?.querySelector('.booking-error'); if (group && !error) { error = document.createElement('small'); error.className = 'booking-error'; error.setAttribute('role', 'alert'); group.appendChild(error); } if (group && error) { group.classList.toggle('is-invalid', show && !ok); error.textContent = show && !ok ? 'Please choose at least one focus area.' : ''; error.hidden = !show || ok; } return ok; }
  ...
  function renderReviewSection() { renderDl('[data-booking-review-required]', requiredRows()); renderDl('[data-booking-review-optional]', optionalRows()); }
  function renderDl(selector, rows) { const list = modal.querySelector(selector); if (!list) return; const visible = rows.filter((row) => row[1]); list.innerHTML = visible.length ? visible.map(([label, value]) => '<div><dt>' + escapeHtml(label) + '</dt><dd>' + escapeHtml(value) + '</dd></div>').join('') : '<div><dt>Optional preparation</dt><dd>Not provided</dd></div>'; }
  function requiredRows() { const a = bookingState.appointment; return [['Session type', a.sessionType], ['Date', a.date ? formatDateDisplay(a.date) : ''], ['Time', a.startTime && a.endTime ? formatTimeRange(a.startTime, a.endTime, a.timezone, a.date) : ''], ['Duration', a.durationMinutes ? a.durationMinutes + ' minutes' : ''], ['Selected focuses', bookingState.bookingReason], ['Full name', bookingState.personalInfo.fullName], ['Email', bookingState.personalInfo.email], ['Phone', bookingState.personalInfo.phone]]; }
  function optionalRows() { const o = bookingState.optionalDetails; return [['Biggest challenge', o.biggestChallenge], ['Desired outcome', o.desiredOutcome], ['Consultant experience', o.consultantExperience], ['AI experience', o.aiExperience], ['Previous attempts', o.previousAttempts], ['Priorities', (o.priorities || []).join(', ')], ['Materials', (o.reviewItems || []).map(labelForReviewItem).join(', ')], ['Files', (o.selectedFileMetadata || []).map((file) => file.name).join(', ')], ['Notes', o.notes]]; }
  function labelForReviewItem(key) { return ({ website: 'Website', portfolio: 'Portfolio', pitchDeck: 'Pitch deck', socialMedia: 'Social media', pdf: 'PDF', other: 'Other' }[key]) || key; }
  function resetSuccessScreen() { modal.classList.remove('is-booking-success'); if (reviewContent) reviewContent.hidden = false; if (successContent) successContent.hidden = true; if (successMessage) { successMessage.hidden = true; successMessage.textContent = ''; } }
  function customerTimeZone() { return Intl.DateTimeFormat().resolvedOptions().timeZone || 'UTC'; }
  function bookingMessage() { const o = bookingState.optionalDetails; return [o.biggestChallenge, o.desiredOutcome, o.previousAttempts, (o.priorities || []).join(', '), o.notes].filter(Boolean).join('\n\n'); }
  function showConfirmMessage(message) { if (!confirmMessage) return; confirmMessage.textContent = message || ''; confirmMessage.hidden = !message; }
  function bookingErrorMessage(status, data) { if (status === 409) return 'That time was just booked. Please go back and choose another available time.'; if (status === 400) return data?.error || 'Please review your booking information and try again.'; if (status >= 500) return 'Google Calendar could not create the booking. Please try again in a moment.'; return data?.error || 'The booking could not be completed. Please try again.'; }
  function showSuccessScreen(result) { createdBooking = result; bookingState.createdBooking = result; bookingState.confirmed = true; bookingState.confirmedAt = new Date().toISOString(); saveBookingState(); updateProgress(7); modal.classList.add('is-booking-success'); if (reviewContent) reviewContent.hidden = true; if (successContent) successContent.hidden = false; const firstName = (bookingState.personalInfo.fullName || '').trim().split(/\s+/)[0]; if (successTitle) successTitle.innerHTML = firstName ? 'You&rsquo;re all set, ' + escapeHtml(firstName) + '!' : 'You&rsquo;re all set!'; if (successSessionType) successSessionType.textContent = (bookingState.appointment.sessionType || '1:1 Vision Strategy Session').toUpperCase(); if (successDetails) { const zone = customerTimeZone(); const start = new Date(result.startTime); const end = new Date(result.endTime); const date = new Intl.DateTimeFormat('en-US', { timeZone: zone, month: 'long', day: 'numeric', year: 'numeric' }).format(start); const time = new Intl.DateTimeFormat('en-US', { timeZone: zone, hour: 'numeric', minute: '2-digit' }).format(start) + ' - ' + new Intl.DateTimeFormat('en-US', { timeZone: zone, hour: 'numeric', minute: '2-digit' }).format(end); successDetails.textContent = bookingState.personalInfo.fullName + ', your ' + bookingState.appointment.sessionType + ' is booked for ' + date + ' at ' + time + ' (' + zone + '). The invitation was sent to ' + bookingState.personalInfo.email + '.'; } if (successPrepCopy) successPrepCopy.textContent = optionalSummary() ? 'We\'ll review the information you shared before your session.' : 'We\'ll use your booking goal to prepare for the conversation.'; showConfirmMessage(''); requestAnimationFrame(() => successTitle?.focus({ preventScroll: true })); }
  async function confirmBooking(event) { if (event) event.preventDefault(); syncPersonalInfo(); syncOptionalDetails(); bookingState.finalDetails.policyAccepted = Boolean(policyInput?.checked); saveBookingState(); if (confirmLocked || !isReviewValid(true)) { updateButtons(); focusFirstInvalid(6); return; } confirmLocked = true; const originalLabel = confirmButton?.textContent || 'Confirm Booking'; if (confirmButton) { confirmButton.disabled = true; confirmButton.textContent = 'Creating Booking...'; } showConfirmMessage('Creating your calendar event...'); const a = bookingState.appointment; try { const response = await fetch('/api/book', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ customerName: bookingState.personalInfo.fullName, customerEmail: bookingState.personalInfo.email, consultation: a.sessionType, bookingDate: a.date, startTime: a.startTime, endTime: a.endTime, customerTimeZone: customerTimeZone(), bookingGoal: bookingState.bookingReason, message: bookingMessage() }) }); let data = {}; try { data = await response.json(); } catch (error) {} if (!response.ok) { showConfirmMessage(bookingErrorMessage(response.status, data)); return; } showSuccessScreen(data); } catch (error) { showConfirmMessage('A network error prevented the booking. Check your connection and try again.'); } finally { confirmLocked = false; if (confirmButton) { confirmButton.textContent = originalLabel; updateButtons(); } } }
  function pad2(value) { return String(value).padStart(2, '0'); }
  function calendarStamp(date, time) { const parts = String(date || '').split('-').map(Number); const clock = String(time || '').split(':').map(Number); if (parts.length !== 3 || clock.length < 2) return ''; return String(parts[0]) + pad2(parts[1]) + pad2(parts[2]) + 'T' + pad2(clock[0]) + pad2(clock[1]) + '00'; }
  function escapeIcs(value) { return String(value || '').replace(/\\/g, '\\\\').replace(/;/g, '\\;').replace(/,/g, '\\,').replace(/\r?\n/g, '\\n'); }
  function downloadCalendarFile() { if (!createdBooking?.eventId) { if (successMessage) { successMessage.hidden = false; successMessage.textContent = 'Calendar details are unavailable. Please use the invitation email.'; } return; } const a = bookingState.appointment; const timezone = a.timezone || 'America/Los_Angeles'; const ics = ['BEGIN:VCALENDAR','VERSION:2.0','PRODID:-//E4LA//Booking//EN','CALSCALE:GREGORIAN','METHOD:PUBLISH','BEGIN:VEVENT','UID:' + escapeIcs(createdBooking.iCalUID || createdBooking.eventId + '@google.com'),'DTSTAMP:' + new Date().toISOString().replace(/[-:]/g, '').replace(/\.\d{3}Z$/, 'Z'),'DTSTART;TZID=' + timezone + ':' + calendarStamp(a.date, a.startTime),'DTEND;TZID=' + timezone + ':' + calendarStamp(a.date, a.endTime),'SUMMARY:' + escapeIcs(a.sessionType || 'E4LA Strategy Session'),'DESCRIPTION:' + escapeIcs('This event was created by E4LA. Google Calendar event ID: ' + createdBooking.eventId),'END:VEVENT','END:VCALENDAR'].join('\r\n'); const blob = new Blob([ics], { type: 'text/calendar;charset=utf-8' }); const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = 'e4la-vision-strategy-session.ics'; document.body.appendChild(link); link.click(); link.remove(); window.setTimeout(() => URL.revokeObjectURL(link.href), 1000); }
  function addToCalendar(event) { if (event) event.preventDefault(); if (!createdBooking?.eventId) { if (successMessage) { successMessage.hidden = false; successMessage.textContent = 'Calendar details are unavailable. Please use the invitation email.'; } return; } const openGoogle = window.confirm('Add to Google Calendar?\n\nChoose OK to open the created Google Calendar event. Choose Cancel to download an ICS file for Apple Calendar or Outlook.'); if (openGoogle) { if (createdBooking.htmlLink) window.open(createdBooking.htmlLink, '_blank', 'noopener,noreferrer'); else if (successMessage) { successMessage.hidden = false; successMessage.textContent = 'Open the Google Calendar invitation from your email.'; } } else downloadCalendarFile(); }
  function openBooking(event) { if (event) event.preventDefault(); syncAppointmentFromCalendar(); if (!hasValidAppointment()) { window.E4LABookingCalendar?.showMessage?.('Please select a date and time before booking.'); document.getElementById('coaching')?.scrollIntoView({ behavior: 'smooth', block: 'start' }); return; } lastFocused = document.activeElement; applyStateToUI(); setBookingStep(1, { force: true }); modal.classList.add('is-open'); modal.setAttribute('aria-hidden', 'false'); document.body.classList.add('booking-modal-open'); }
  function closeBooking(event) { if (event) event.preventDefault(); modal.classList.remove('is-open'); modal.setAttribute('aria-hidden', 'true'); document.body.classList.remove('booking-modal-open'); if (lastFocused?.focus) lastFocused.focus({ preventScroll: true }); }
  function openNotice(event) { if (event) event.preventDefault(); syncPersonalInfo(); syncOptionalDetails(); if (!isStep2Valid(true)) { focusFirstInvalid(2); return; } lastNoticeTrigger = event.currentTarget; notice?.classList.add('is-open'); notice?.setAttribute('aria-hidden', 'false'); document.body.classList.add('booking-notice-open'); requestAnimationFrame(() => noticeAnswer?.focus({ preventScroll: true })); }
  function closeNotice(event, returnFocus = true) { if (event) event.preventDefault(); notice?.classList.remove('is-open'); notice?.setAttribute('aria-hidden', 'true'); document.body.classList.remove('booking-notice-open'); if (returnFocus && lastNoticeTrigger?.focus) lastNoticeTrigger.focus({ preventScroll: true }); }
  ...
  modal.querySelector('[data-open-optional-notice]')?.addEventListener('click', openNotice); noticeCloseButtons.forEach((button) => button.addEventListener('click', (event) => closeNotice(event, true))); noticeAnswer?.addEventListener('click', (event) => { closeNotice(event, false); setBookingStep(3, { force: true }); }); noticeSkip?.addEventListener('click', (event) => { closeNotice(event, false); setBookingStep(6, { force: true }); }); confirmButton?.addEventListener('click', confirmBooking); addCalendarButton?.addEventListener('click', addToCalendar); returnHomeButton?.addEventListener('click', (event) => { event.preventDefault(); closeBooking(event); window.scrollTo({ top: 0, behavior: 'smooth' }); }); summaryEditButtons.forEach((button) => button.addEventListener('click', openAppointmentEditor)); appointmentCancelButtons.forEach((button) => button.addEventListener('click', (event) => closeAppointmentEditor(event, { returnFocus: true }))); appointmentSaveButton?.addEventListener('click', saveAppointmentEditor); modal.addEventListener('click', (event) => { if (event.target.classList.contains('booking-modal__backdrop')) closeBooking(event); });
  document.addEventListener('keydown', (event) => { if (notice?.classList.contains('is-open')) { if (event.key === 'Escape') { closeNotice(event, true); return; } if (event.key === 'Tab') { const focusable = getNoticeFocusable(); const first = focusable[0]; const last = focusable[focusable.length - 1]; if (!first) return; if (event.shiftKey && document.activeElement === first) { event.preventDefault(); last.focus(); } else if (!event.shiftKey && document.activeElement === last) { event.preventDefault(); first.focus(); } } return; } if (!modal.classList.contains('is-open')) return; if (event.key === 'Escape') closeBooking(event); });
  window.E4LABookingFlow = { refreshSummary() { bookingState = loadBookingState(); applyStateToUI(); }, getState: () => clone(bookingState) };
  applyStateToUI();
})();
